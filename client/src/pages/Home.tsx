...
function AnalyseScreen({ state, simulation, windowValues, sharedMinute, setSharedMinute, selectedSensor, guidanceRules, setGuidanceRules, lockedStrategy, setLockedStrategy, decisionNote, setDecisionNote, analysisChapter, setAnalysisChapter }: {
  ...
  const recommendedStrategy = strategies.find((strategy) => strategy.name === guidance.recommendedStrategy) ?? strategies[0];
  const displayedStrategy = activeStrategy ?? recommendedStrategy;
  ...
  const guidancePhaseBands = guidance.phases.map((phase) => ({
    start: Math.floor(phaseRange[phase.id].start / 30),
    end: Math.ceil(phaseRange[phase.id].end / 30),
    label: phase.label,
    active: phase.id === activePhase,
    color: phase.id === "before" ? "#0f6c62" : phase.id === "during" ? "#b64d32" : "#b28a2f",
  }));
  ...
  const guidanceChartSeries = [
    { label: "Outdoor PM₂.₅ boundary", values: simulation.externalMinute.filter((_, i) => i % 30 === 0), color: "#b64d32", width: 2.1 },
    { label: "Current indoor response", values: simulation.indoorMinute.filter((_, i) => i % 30 === 0), color: "#202827", dashed: true },
    { label: `${displayedStrategy.name} strategy`, values: displayedStrategy.curve.filter((_, i) => i % 30 === 0), color: displayedStrategy.name === guidance.recommendedStrategy ? "#0f6c62" : "#b28a2f", width: 2.4 },
  ];
    ...
    <div className="screen-grid grid-analyse">
      ...
      <Panel className={`analysis-panel guidance-panel ${analysisChapter === 3 ? "chapter-active" : "chapter-muted"}`} eyebrow="4. Ventilation guidance" action={<div className="button-row"><button className="secondary-button" onClick={() => setRulesOpen(true)}><Settings2 size={13} /> What changes this?</button><button className="primary-button" onClick={() => setExportOpen(true)}><Download size={13} /> Export</button></div>}>
        <div className="guidance-layout">
          ...
          <aside className="guidance-evidence" aria-label="Recommendation evidence"><div className="guidance-evidence-heading"><div><span>Selected recommendation evidence</span><h4>{guidance.phases.find((phase) => phase.id === activePhase)?.label} · {displayedStrategy.name}</h4></div><small>{lockedStrategy ? "Comparison locked" : "Select a recommendation or preview a strategy"}</small></div><div className="guidance-inputs"><div><span>Selected event</span><b>{CASES[state.selectedCase].title}</b></div><div><span>Shared context</span><b>{selectedSensor} · {Math.floor(sharedMinute / 60)}h {sharedMinute % 60}m</b></div></div><div className="guidance-chart-card"><ScientificChart series={guidanceChartSeries} markerIndex={sharedMarker} range={sampledRange} bands={guidancePhaseBands} height={260} chartWidth={900} yLabel="" xLabel="" showReadout={false} pad={{ left: 18, right: 0, top: 8, bottom: 14 }} /></div><div className="guidance-current-summary"><article><span>Phase cue</span><b>{guidance.phases.find((phase) => phase.id === activePhase)?.label}</b></article><article><span>Strategy peak</span><b>{displayedStrategy.peak.toFixed(1)} <small>µg/m³</small></b></article><article><span>Peak reduction</span><b>{Math.max(0, displayedStrategy.peakReduction).toFixed(0)}%</b></article><article><span>Decision state</span><b>{displayedStrategy.name === guidance.recommendedStrategy ? "Recommended" : "Comparison"}</b></article></div><div className="strategy-bars">{strategies.map((strategy) => {
            ...
            return <button type="button" className={`strategy-row ${isRecommended ? "recommended" : ""} ${isPreviewed ? "previewed" : ""}`} key={strategy.name} aria-pressed={isLocked} onMouseEnter={() => setHoveredStrategy(strategy.name)} onMouseLeave={() => setHoveredStrategy(null)} onFocus={() => setHoveredStrategy(strategy.name)} onBlur={() => setHoveredStrategy(null)} onClick={() => { setLockedStrategy((value) => value === strategy.name ? null : strategy.name); setAnalysisChapter(3); }}><span className="strategy-name"><b>{strategy.name}</b><small>{isLocked ? "Locked comparison" : isRecommended ? "Recommended" : "Preview curve"}</small></span><span className="strategy-track" role="img" aria-label={`${strategy.name}: ${strategy.peak.toFixed(1)} micrograms per cubic metre`}><span className="strategy-fill" style={{ width: `${barWidth}%` }} /></span><span className="strategy-value"><strong>{strategy.peak.toFixed(1)}</strong><small>↓ {Math.max(0, strategy.peakReduction).toFixed(0)}%</small></span></button>;
